...
export default function Nacionales() {
  ...
  // Cargar noticias nacionales de la API
  useEffect(() => {
    const loadNoticias = async () => {
      try {
        setLoading(true);
        const data = await apiService.getNoticias({ categoria: 'Nacionales' });
        setNoticias(data);
      } catch (err) {
        console.error('Error cargando noticias nacionales:', err);
        setError('Error al cargar las noticias nacionales');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="actualidad-container">
        ...
        <div style={{ textAlign: 'center', padding: '50px' }}>
          <h2>Cargando noticias nacionales...</h2>
        </div>
      </div>
    );
  }
  ...
  if (!noticias || noticias.length === 0) {
    return (
      <div className="actualidad-container">
        ...
        <div style={{ textAlign: 'center', padding: '50px' }}>
          <h2>No hay noticias nacionales disponibles</h2>
        </div>
      </div>
    );
  }
          ...
          <article key={noticia.id} className="noticia-card">
            ...
            >
              <img
                ...
                onError={(e) => {
                  e.target.src = "https://via.placeholder.com/400x200/333333/ffffff?text=Noticia+Nacional";
                }}
              />
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <img
              ...
              className="modal-img"
              onError={(e) => {
                e.target.src = "https://via.placeholder.com/600x300/333333/ffffff?text=Noticia+Nacional";
              }}
            />
